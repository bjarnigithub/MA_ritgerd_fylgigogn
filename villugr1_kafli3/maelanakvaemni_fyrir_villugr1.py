...
def processdata(listcontainingsentences):
  aðaltengingar = ["og", "en", "eða", "heldur", "enda", "ellegar"]  
...
def compareoutputs(humanoutput,computeroutput):
  ...
  for sentence in computeroutput:
    if not sentence == humanoutput[index_of_current_sentence_in_computeroutput]: 
      ...
      # finna fjölda true_positives í setnigum sem hafa ekki villur
...
def findtptnfpandfn(list_of_incorrect_sentences, true_positive_in_compareoutputs):
  ...
  adaltengingar = ["og", "en", "eda", "heldur", "enda", "ellegar"]
  ...
  for tuple in list_of_incorrect_sentences: 
    ...
    for element in newer_splitter_sentence: 
      if element == "BOUNDARY": 
        ...
        ############################# UPPFÆRSLA #############################
        # þetta er til þess að það komi ekki upp villa ef það er aðaltenging í 
        # nærumhverfi þess sem hefur verið greint sem aðasetninga mark
...
def displayresults(deep_copy_of_list_of_incorrect_sentences, precision, recall, f_measure, list_of_false_positive_examples, list_of_false_negative_examples): 
  ...
  results_filename = input("Hvað á skráin með niðurstöðunum að heita? ")
...
def main(): 
  correct_version_filename = input("Hvað heitir skráin með gullstaðlinum? ")
  splitter_version_filename = input("Hvað heitir skráin sem inniheldur úttakið úr splitter.py? ")
